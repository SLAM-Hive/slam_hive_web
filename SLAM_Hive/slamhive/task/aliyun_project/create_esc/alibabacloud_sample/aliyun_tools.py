...
class Esc_Create:
    ...
    def describe_instance_status(
        ...
        while True:
            ...
            if now_start >= all_size:
                break
        # print("status")
        # print(all_instance_status_list)
    ...
    def check_instance_status(instance_status_list):
        running_number = 0
        # print("instance_status_list len:", len(instance_status_list))
        for i in range(len(instance_status_list)):
            # print(instance_status_list[i].status)
    ...
    def check_instance_status_stopped(instance_status_list):
        stopped_number = 0
        for i in range(len(instance_status_list)):
            if instance_status_list[i].status == "Stopped":
                stopped_number += 1
        if stopped_number == len(instance_status_list):
            return True
        else :
            return False
    ...
    def stop_instances(
        ...
        try:
            # 复制代码运行请自行打印 API 的返回值
            client.stop_instances_with_options(stop_instances_request, runtime)
        except Exception as error:
            # 如有需要，请打印 error
            print(error)
    ...
    def delete_image(
        ...
        try:
            # 复制代码运行请自行打印 API 的返回值
            client.delete_image_with_options(delete_image_request, runtime)
        except Exception as error:
            # 如有需要，请打印 error
            # UtilClient.assert_as_string(error.message)
            print(error)
